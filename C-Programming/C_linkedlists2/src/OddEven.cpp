...
int * oddeven_sll(struct oddevennode *head){
	...
	struct oddevennode *temp = head,*listCurrent=head, *oddCurrent=NULL, *evenCurrent=NULL;
	int *output = (int*)calloc(2,sizeof(int));
	
	while (listCurrent != NULL)
	{
		if (listCurrent->data % 2)//odd
		{
			
			if (output[0] == 0)  //first oddnode
			{
				
				oddCurrent = listCurrent;
				listCurrent = listCurrent->next;
				output[0]++;
				continue;
			}
				oddCurrent->random = listCurrent;
				oddCurrent = listCurrent;
				output[0]++;
		...
		else  //even
		{
			...
			if (output[1] == 0)//first even node
			{
				evenCurrent = listCurrent;
				output[1]++;
				listCurrent = listCurrent->next;
				continue;
			}
			
				evenCurrent->random = listCurrent;
				evenCurrent = listCurrent;
				output[1]++;
		}

		//move listcurrent pointer
		listCurrent = listCurrent->next;


	}
	//making last even and odd randoms to point null
	if (output[1])  //if evennodes found
	evenCurrent->random = NULL;
	if (output[0])//if oddnodes found
	oddCurrent->random = NULL;
