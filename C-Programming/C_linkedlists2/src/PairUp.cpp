...
Alll students are very much excited and want to roam the whole zoo with their Close friends ,
and everyone formed a line where close friends stand one behind other ,But the class teacher has some different plans in her mind ,
she want everyone in the class to know each other in the picnic as usually even in the school the same set of students talk with each other 
and usually some introverts are left out with no one to talk .

She wanted to pairUp students in the way that everyone gets paired up randomly ,
But Doing randomly became difficult as she dosent have a Good Random number generator :D .
She now decided that in the Line the first student ,gets paired up with the last student , 
the second student gets paired up with the second student from last ,third student with last third student and so on . 
This way she wants to pair up every student and form a new Line of students .
...
struct ListNode * recursionRevList2(struct ListNode *head)
{
	if (head->next == NULL)
		return head;

	//link last returned heads next to present head
	recursionRevList2(head->next)->next = head;
...
void pairUp(struct ListNode *head){
	...
	struct ListNode *tempNode = head, *listmid=head,*midlist2,*ReverselistHead,*listCurrent,*listPrevious,*Revlisthead;


	//dividing list from mid next
	while (tempNode->next != NULL)
	{
		count++;

		if (count % 2 == 0)  //pointing mid of the list
...
//reverse the list	
	...
	recursionRevList2(Revlisthead)->next=NULL;
	...
	{
		if (ReverselistHead != NULL)//merge the reversed list
		{
			tempNode = ReverselistHead;
			ReverselistHead = ReverselistHead->next;
			tempNode->next = listCurrent->next;
			listCurrent->next = tempNode;
			listCurrent = tempNode->next;
		}
		else
			break;
	}
