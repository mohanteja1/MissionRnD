...
For each test case, you will be given two arrays, an input array consisting of numbers and output array of - 1's of the same length.
For every index, if the number in the input array is perfect then set the corresponding number in output array to 1. 
If not, then set the corresponding number to 0.
...
void isPerfect(long input[], int output[], int length)
{
	...
	for (i = 0; i < length; i++)
	{
		m = input[i];
		sum = 0;
		for (j = 1; j <= m / 2; j++)//loop for calculating  factor sum
		{
		...
		if (sum == m) //check number if perfect or not
		{
			output[i] = 1;//updating output array
		}
		else
			output[i] = 0;
	}
