...
unsigned int swapBits(unsigned int number, int p1, int p2, int n)
{
	...
	if (!((0 <= p1 && p1 <= 32) && (0 <= p2 && p2 <= 32) &&  n > 0 && (p1 + n <= p2)) || (p2<0 || p1<0 || number<0 || n<0))
		return 0;


	unsigned  long int m, m1, m2, m3, m4, word, p1v, p2v, pv,numtemp;
	m1 = 4294967295;
	//m1 = ~m1;
	numtemp = p1v = p2v = (unsigned)number;
	word = 8 * sizeof(unsigned long int);
