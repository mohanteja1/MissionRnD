...
int minOfthree(int one, int two, int three)
{
	if (one < two)
	{
		if (one < three)
...
int minBallDistanceRec(struct student *leaderStudent)
{
	int count1 = 0, count2 = 0, count3 = 0;
	if (leaderStudent == NULL)
		return 2;
	if (leaderStudent->first == NULL&&leaderStudent->second == NULL && leaderStudent->third == NULL)
		return 1;

	//printf("%c\n", leaderStudent->letter);

	count1 = minBallDistanceRec(leaderStudent->first);

	count2 = minBallDistanceRec(leaderStudent->second);

	count3 = minBallDistanceRec(leaderStudent->third);

	return 1 + minOfthree(count1, count2, count3);
...
int maxBallDistanceRec(struct student *leaderStudent)
{
	int count1 = 0, count2 = 0, count3 = 0;
	...
	count1 = maxBallDistanceRec(leaderStudent->first);
	count2 = maxBallDistanceRec(leaderStudent->second);
	count3 = maxBallDistanceRec(leaderStudent->third);

	if (count1 == 0 && count2 == 0 && count3 == 0)
		return 1;
	
	return 1 + maxOfThree(count1, count2, count3);
...
void findBallDistance(struct student *leaderStudent, int *minBallDistance, int *maxBallDistance)
{
	...
	*minBallDistance = minBallDistanceRec(leaderStudent);
	//printf("max\n");
	*maxBallDistance = maxBallDistanceRec(leaderStudent);

	//printf("min=%d", minBallDistanceRec(leaderStudent));


    //	printf("max=%d", maxBallDistanceRec(leaderStudent));
