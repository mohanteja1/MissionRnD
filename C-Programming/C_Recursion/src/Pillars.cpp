...
int isInBoard1(int x, int y,int n,int m )
{
	if (x < 0 || x >= n || y < 0 || y >= m)
		return 0;
	else
...
void print1(int *arr, int n,int m)
{
...
int maxOfFour(int count1, int count2, int count3, int count4,int indexOrNotMax)
{
	if (count1 == 0 && count2 == 0 && count3 == 0 && count4 == 0)
		return 0;

	int max = count1, index = 1;
...
int copyPath(int *blocks, int n, int m, int currentX, int currentY,int lastValue, int *maxPath, int maxCount, int currentIndex)
{

	if (currentIndex == maxCount&&isInBoard1(currentX, currentY, n, m) && blocks[(currentX)*m + (currentY)]>lastValue)
	{
		
		maxPath[currentIndex] = blocks[(currentX )*m + (currentY)];
		//printf("max[%d] == %d\n", currentIndex, maxPath[currentIndex]);
		return 1;
	}

	if (isInBoard1(currentX, currentY, n, m) && blocks[(currentX)*m + (currentY)]>lastValue)
	{
		maxPath[currentIndex] = blocks[(currentX)*m + (currentY)];
		//printf("max[%d] == %d\n",currentIndex, maxPath[currentIndex]);
		if (copyPath(blocks, n, m, currentX-1, currentY, blocks[(currentX)*m + (currentY)], maxPath, maxCount, currentIndex + 1))
			return 1;

		if (copyPath(blocks, n, m, currentX+1, currentY, blocks[(currentX)*m + (currentY)], maxPath, maxCount, currentIndex + 1))
			return 1;
		if (copyPath(blocks, n, m, currentX, currentY-1, blocks[(currentX)*m + (currentY)], maxPath, maxCount, currentIndex + 1))
			return 1;
...
int findPath(int *blocks, int n, int m, int currentX, int currentY)
{
...
//four counts choose maximum
	if (isInBoard1(currentX - 1, currentY, n, m))
		//nextNum                    >           //current num
		if (blocks[(currentX - 1)*m + (currentY)] > blocks[(currentX)*m + (currentY)]){
			//printf("up\n");
			count1 = findPath(blocks, n, m, currentX - 1, currentY);
			flag = 1;
		}
	...
	if (isInBoard1(currentX + 1, currentY, n, m))
		...
		{
			//printf("down\n");
			count2 = findPath(blocks, n, m, currentX + 1, currentY);
			flag = 1;
		}


	if ( isInBoard1(currentX , currentY -1, n, m))
		//nextNum                    >           //current num
		if (blocks[(currentX)*m + (currentY - 1)] > blocks[(currentX)*m + (currentY)])
		{
			//printf("left\n");
			count3 = findPath(blocks, n, m, currentX, currentY - 1);
			flag = 1;
	...
	if (isInBoard1(currentX , currentY+1, n, m))
		...
		{
			//printf("right\n");
			count4 = findPath(blocks, n, m, currentX, currentY + 1);
			flag = 1;
		}
	...
	//reached highest number
	if (flag == 0)		
		return 0;

	
	return 1 + maxOfFour(count1, count2, count3, count4,0);
...
int * pillars_max_jumps(int *blocks, int n, int m, int *jumps_count){
	...
	int maxCount = 0, tempCount = 0, tempIndex=0, MaxIndex=0;
	int LengthOfMatrix = n*m;

	//print1(blocks, n, m);

	int nullFlag = 0;

	while (tempIndex < LengthOfMatrix)
	{
		...
		tempCount = findPath(blocks, n, m, tempIndex / m, tempIndex%m);
		nullFlag = 1;
     ...
     *jumps_count = maxCount;

	if (nullFlag == 0)
		 return NULL;

	int *maxPath = (int *)calloc((maxCount+1),sizeof(int));
	
	maxPath[0] = blocks[MaxIndex];
	
	tempCount = copyPath(blocks, n, m, MaxIndex / m, MaxIndex%m,0, maxPath,maxCount,0);
	
//	print1(maxPath, 1, maxCount+1);
