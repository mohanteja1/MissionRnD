...
long long kingSteps(long long destX, long long destY,int currentx, int currenty)
{
	//printf("k= %lld,%lld", destX, destY);
	if (destX == destY){
		return destX;
	}
	else
	if (destX < destY)
		return destX + (destY - destX);
	else
		return destY + (destY - destX);

}

long long bishopSteps(long long destX, long long destY, int currentx, int currenty)
{
	//printf("b = %lld,%lld\n", destX, destY);
...
long long rookSteps(long long destX, long long destY,int currentx, int currenty)
{
	//printf("R = %lld,%lld", destX, destY);
...
long long minimumPieceMoves(long long sourceX, long long sourceY, long long destX, long long destY, char piece){
	if (sourceX < 1 || sourceY < 1 || destY < 1 || destX < 1)
		return -1;

	//printf("%lld,%lld\n", llabs(destX - sourceX), llabs(destY - sourceY));
	
	
	switch (piece)
	{
	case 'K':
		return kingSteps(llabs(destX - sourceX), llabs(destY - sourceY), 0, 0);
	
	case 'R':
		return rookSteps(llabs(destX - sourceX), llabs(destY - sourceY), 0, 0);

	case 'B':
		
		return bishopSteps(llabs(destX - sourceX), llabs(destY - sourceY), 0, 0);
