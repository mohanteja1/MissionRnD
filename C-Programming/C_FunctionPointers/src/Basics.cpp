...
void task2() {
	...
	char string[] = "mohanteja";

	invokePrint2(printMessage, string);
...
char * subString(char *str, int a, int b){
	...
	char *string = (char*)malloc(sizeof(char)*(abs(b - a) + 1));
	int i = a-1;
	int j = 0;
	while (str[i] != 0 && i<b)
	{
...
void task3() {
	...
	char string[] = "mohanteja is jonsnow";
	printf("%s", invokeSubStr(subString,string, 0, 9));
}
