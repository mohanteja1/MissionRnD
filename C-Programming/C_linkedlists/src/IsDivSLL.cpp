...
int isDivisibleSLL(struct charNode * head, int key){
	...
	int num = 0,revNum=0, negativeFlag = 0;
	...
	while (temp != NULL)
	{
		num = num * 10 + ((temp->letter) - 48);  //calculate number
		revNum = revNum + ((temp->letter) - 48)*(pow(count));//calculate reverse number
		temp = temp->next;
		count++;
	}


	if (negativeFlag)   //check negative sign flag
	{
		num *= -1;
		revNum *= -1;
	}
