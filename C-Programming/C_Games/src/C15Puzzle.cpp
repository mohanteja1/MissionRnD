...
void c15Puzzle_main(){
	...
	char playerName[10];// = "mohanteja";
	printf("Enter the player1 name:");
	scanf_s("%s", playerName, 10);

	struct User *player = createUser(playerName);
	...
	printf("\nselect difficulty level from 1 to 3\n");
	int difficultyLevel;
	scanf_s("%d", &difficultyLevel);
	//int tabValues[4][4];
	int *tabValues2;
	...
	initializeBoard(board, tabValues2, rows, cols);
