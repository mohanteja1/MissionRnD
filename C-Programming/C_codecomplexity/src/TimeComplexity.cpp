...
char* function8(int n) {
	...
	int k = 0;
	for (i = 1; i <= n; i++){               //n
		for (j = 1; j <= n; j *= 2) {           //log n +1
			for (k = 2; k < 1000; k++){          // constant
...
char* function10(int n) {
	...
	for (i = 1; i <= n; i += 2) {
		for (j = 1; j <= power(n, i); j += 2) {
			//some O(1) tasks
			// this block  gonna run half of sum of n's of even powers upto n ie..,(1+ n^2 + n^4 + n^6 ....n^n)/2
