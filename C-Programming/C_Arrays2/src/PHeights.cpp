...
int* getLeaderIndex(int *arr, int len){
	...
	if (arr == NULL || len < 0||len>100000)
		return NULL;
	int i, count = 1, morecount = 0, required_index = 0;
	...
	for (i = 0; i < len; i++)
	{
		...
		//calculate count of first i after ascending and descending series in between
		for (int j = i-1; j >=0; j--)
		{
		    	if (arr[j] < arr[i])
			{
		...
		while (arr[i]<arr[i + 1])  //skip ascending series in between
		{
				i++;
			count++;
		}
		...
		while (arr[i]>arr[i + 1])             //skip descending numbers in between 
		{
			i++;
		}
	...
	//double check morecount
	count = 0;
	for (int j = required_index - 1; j >= 0; j--)
	{

		
		if (arr[j] < arr[required_index])
		{
			count++;
