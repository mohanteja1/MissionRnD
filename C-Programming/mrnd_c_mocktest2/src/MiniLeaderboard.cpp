...
void stringcpy(char *string, int copylen, char *out){
...
struct student *createNode(char *sname)
{
	struct student *newnode = (struct student *)malloc(sizeof(struct student) * 1);
	stringcpy(sname,3, newnode->studentName);
	return newnode;
}
struct student *search(char *name,struct student *head)
{
	while (head != NULL)
	{
		if (!strcmp(head->studentName, name))
			return head;
		head = head->nextnode;
	}
	//if name not present
...
struct student * insertSubject(struct student *root, char *lesson,int subject,struct student *head)
{
	//read subject list
	    //if new name create new node and insert it in null
	char tempname[4];
	int index = 0;
	struct student *currentStu = NULL;
	while (lesson[index]){
		//get student node
		if (lesson[index] == 'a' || lesson[index] == 'b' || lesson[index] == 'c')
		{
			stringcpy(&lesson[index], 3, tempname);
			currentStu = search(tempname,head);
			index += 3;
		}
		if (lesson[index] == ',')
			index++;
