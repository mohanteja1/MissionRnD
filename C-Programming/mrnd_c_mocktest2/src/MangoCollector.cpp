...
//returns 
int shortmangoNode(struct node *city,int currentCount, struct node *endnode,int k)
{
	if (currentCount == k)
		return 1;
	
	currentCount += city->data;

	int count1 = shortmangoNode(city->left, currentCount, endnode, k);
	int count2 = shortmangoNode(city->right, currentCount, endnode, k);
