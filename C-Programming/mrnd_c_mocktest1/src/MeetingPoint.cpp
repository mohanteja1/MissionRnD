...
struct CityNode * meetingPoint(struct CityNode * deepikaStartCity, struct CityNode *mayaStartCity, int *numberOfCitiesMayaShouldTravel) {
	...
	struct CityNode *tempDepika,*currentMaya=mayaStartCity;
	int count=0;
	//struct CityNode *meetingPoint = (struct CityNode *)malloc(sizeof(struct CityNode) * 1);
	while (currentMaya != NULL)
	{
		//printf("%s\n", currentMaya->city);
		tempDepika = deepikaStartCity;
		while (tempDepika != NULL)
		{
			//printf("temp dipeka :%s\n", currentMaya->city);
			if (strcmp(tempDepika->city,currentMaya->city)==0)
			{
				//printf("deepika %s\n", tempDepika->city);
			...
			tempDepika = tempDepika->nextCity;
		}
		count++;
		currentMaya = currentMaya->nextCity;
	}
